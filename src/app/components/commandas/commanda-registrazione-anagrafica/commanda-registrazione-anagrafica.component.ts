...
@Component({
  selector: 'app-commanda-registrazione-anagrafica',
  templateUrl: './commanda-registrazione-anagrafica.component.html',
  styleUrls: ['./commanda-registrazione-anagrafica.component.css']
})
export class CommandaRegistrazioneAnagraficaComponent implements OnInit {
...
=======
   public visibilitiAnagrafica = true;
...
<<<<<<< HEAD
                    ...
                    if(c1 === 'commanda' && c2 === 'RegistraAnag') {
                      if(c3 === 'new') {
...
// verifico se provengo da prodotti (stesso url di nuova registrazione) e effettuo la lettura
                             this.idComma = parseInt(localStorage.getItem('idCommanda'));
=======
      ngOnInit(): void {
                ...
                onSelectionChange(tipo: string)   {
                  ...
                  switch (tipo) {
                            case 'Clienti':
                                ...
                                this.visibilitiAnagrafica = true;
                                this.commandaw = new Commandaw();
                                break;
                            case 'Personale Sanfra':
                                this.visibilitiAnagrafica = false;
                                this.visiblePrenotazioni = false;
                                this.visibleSanfra = true;
                                this.NuovaCommanda(this.idlogged);
                            ...
                            case 'Prenotazioni':
                              ...
                              //  this.NuovaCommanda(this.idlogged);
                                ...
                                this.CancellaCommandawdaPrenotazione(this.idlogged);  // cancello e ricreo le righe prodotti a zero qta
...
async CancellaCommandawdaPrenotazione(id: number) {
...
async registraProdotti() {
  ...
  this.commandaw.importoCoperto = this.commandaw.numPersone * this.manifestazione.impCoperto;
      ...
      resp => {
             if(resp['rc'] === 'ok') {
              //   alert('commandaw aggiornata correttamente - ' + '\n' + ' ora vado a inserire i prodotti');
               ...
               alert('vado a prodotti: ' + this.routenavigate)
...
async aggiornaCommandaw(commandaw: Commandaw) {
  ...
  let rc = await this.commandawService.update(commandaw).subscribe(
    ...
    error => {
         alert('sono in aggiornaCommandaw - error');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('aggiornaCommandaw - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
  ...
  let rc =  await this.prodottoService.getAll().subscribe(
   ...
   //  alert('commandaW - creaarigheCommanda');
     ...
     for(let prodo of this.prodotti) {
       ...
       this.prg += 1;
       this.keywidriga = (this.commandaw.id * 100) + this.prg;

       this.commandawriga.id  = this.keywidriga;
...
// idGiornata: number, inservizio: string
  ...
  let rc = await this.personaService.getPersonabyinServizio(idGiornata, inServizio).subscribe(
   ...
   error => {
        alert('sono in loadPpersonaleSanfra - error');
        this.isVisible = true;
        this.alertSuccess = false;
...
onSelectPrenotazione(prenotazione: Prenotazione) {
 ...
 // aggiorno anagrafica della commanda
...
this.loadAllrighecommandaw(this.idlogged);


  this.visiblePrenotazioni = false;
  this.visibilitiAnagrafica = true;
  // carico le righe della commanda per
  this.loadProdottiPrenotati(prenotazione.id);

}


async loadAllrighecommandaw(id: number) {

  console.log(`loadprenotazioni - appena entrato id: ` + id);
  let rc = await this.commandawrigaService.getAllbyCommandaw(id).subscribe(
   resp => {
          if(resp['rc'] === 'ok') {
   ...
   error => {
        alert('sono in loadAllrighecommandaw - error');
        this.isVisible = true;
        this.alertSuccess = false;
        console.log('loadAllrighecommandaw - errore: ' + error);
        this.type = 'error';
        this.Message = error.message;
        this.alertSuccess = false;
        this.showNotification(this.type, this.Message);
...
async loadProdottiPrenotati(id: number) {
  ...
  let rc = await this.prenotazioneprodottoService.getallProdbyprenotazione(id).subscribe(
    response => {
        if(response['rc'] === 'ok') {
          ...
          this.prg = 0;
          this.keywidriga = 0;
          for(let prodo of this.prenotazioneprodotti) {
             for(let wriga of this.commandawrighe) {
               if(prodo.idprodotto == wriga.idProdotto) {
                  wriga.qta = prodo.qta
                  this.aggiornaCommandawriga(wriga);
               }
            }
          }
        }
  ...
  async inserisciCommandawriga(commandawriga: Commandawriga) {

    let rc = await this.commandawrigaService.createCommandawriga(commandawriga).subscribe(
      response => {
          if(response['rc'] === 'ok') {
            //
          }
  ...
  async aggiornaCommandawriga(commandawriga: Commandawriga) {
...
<<<<<<< HEAD
                  async loadManifestazione(id: number) {
                    console.log(`---->     loadManifestazione - appena dentro`);
                    let res = await this.manifestazioneService.getManifestazione(id).subscribe(
                    response => {
                        if(response['rc'] === 'ok') {
                          this.manifestazione = response['data'];
                        }
...
async NuovaCommanda(id: number) {
  console.log(`nuovaCommanda - appena entrato`);
  let rc = await this.commandawService.getCommandaw(id).subscribe(
   resp => {
     console.log('fatto lettura commanda: ' + id + ' rc: ' + resp['rc']);
           if(resp['rc'] === 'ok') {
            this.CancellaCommandaw(id);
...
async CancellaCommandaw(id: number) {
  ...
  let rc = await this.commandawService.deleteCommandaw(id).subscribe(
    resp => {
           if(resp['rc'] === 'ok') {
              this.createCmmanda();
           }
...
async createCmmanda() {
  ...
  let rc = await this.commandawService.createCommanda(this.commandaw).subscribe(
    resp => {
           if(resp['rc'] === 'ok') {
             console.log('commandaw inserita correttamente -  ora vado a inserire i prodotti ' + this.commandaw.id);
             this.registraCommandawrighe();
...
async loadPrenotazioni(id: number) {
  ...
  let rc = await this.prenotazioneService.getPrenotazinidaEvaderebyday(id).subscribe(
   resp => {
          if(resp['rc'] === 'ok') {
            this.prenotazioni = resp['data'];
          }
