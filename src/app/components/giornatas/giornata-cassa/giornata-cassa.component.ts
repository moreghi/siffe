// attenzione dal 23/02/2022 nuova versione. per effettuare modifiche prendo i dati da cassa e li metto su cassaw
// effettuato modifiche aggiorno cassa. abbandono di modificare i dati su giornata che andranno tolti
                 ...
                 resp => {
                  ...
                  //        this.conteggiaValori();
                        if(this.giornata.cassaInizio === this.giornata.cassaAttuale && this.giornata.cassaInizio === 0){
                         ...
                         // vecchio metodo dove salvavo su giornata
                        // i campi dellla cassa su girnata andranno tolti perchè ho creato la tabella cassa
                        //    this.loadCassaw(id);

                        if(this.giornata.cassaInizio === this.giornata.cassaAttuale) {
                            this.tipoCassa = 'I';   // carico i dati attuali da cassa
                        }
                        if(this.giornata.cassaInizio !== this.giornata.cassaAttuale) {
                          this.tipoCassa = 'F';   // carico i dati attuali da cassa
                        }
                        this.loadCassawfromCassa(this.tipoCassa, id);
       ...
       async   azzeroCassaw(id: number) {
              let rc = await this.cassawService.azzeraCassa(id).subscribe(
                ...
                error => {
                     ...
                     this.isVisible = true;
                     this.alertSuccess = false;
                     console.log('azzeroCassaw - errore: ' + error);
                     this.type = 'error';
                     this.Message = error.message;
                     this.alertSuccess = false;
                     this.showNotification(this.type, this.Message);
...
async Salva() {
  ...
  if(this.giornata.statoCassa === 0 || this.giornata.statoCassa === 1) {
    ...
    this.salvoCassaIniziale(this.idGiornata);

  }
  if(this.giornata.statoCassa > 1  ) {

    this.salvoCassaFinale(this.idGiornata);
...
async salvoCassaIniziale(id: number) {
  console.log('salvocassaIniziale - appena entrato')
  let rc = await this.cassawService.getbyidGiornata(id).subscribe(
    resp => {
          if(resp['rc'] === 'ok') {
            ...
            this.importoCassa = 0;
            console.log('SalvoCassaIniziale ---------- importo cassa -------------  ' + JSON.stringify(this.importoCassa));
            for(let cassaw of this.cassew) {
              this.aggiornaCassaIniziale(cassaw, id);
              }
            this.salvaimportoCassa(id, this.tipoCassa);
            this.azzeroCassaw(this.giornata.id);
            }

          },
    error => {
         alert('sono in loadCassawfromCassa');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('loadCassawfromCassa - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
async salvoCassaFinale(id: number) {

  let rc = await this.cassawService.getbyidGiornata(id).subscribe(
    resp => {
          if(resp['rc'] === 'ok') {
            this.cassew = resp['data'];
            console.log('SalvoCassaFinale ----------------------- cassaw ' + JSON.stringify(this.cassew));
            this.importoCassa = 0;
            for(let cassaw of this.cassew) {
                this.aggiornaCassaFinale(cassaw, id);
              }
            this.salvaimportoCassa(id, this.tipoCassa);
            this.azzeroCassaw(this.giornata.id);
            }
...
async aggiornaCassaIniziale(cassaw: Cassaw, id: number) {

  let rc = await this.cassaService.getbyidTagliaeGiorno(id, cassaw.idTaglia).subscribe(
    resp => {
          if(resp['rc'] === 'ok') {
            this.cassa = resp['data'];
            console.log('aggiornaCassaIniziale ----------------------- cassa ' + JSON.stringify(this.cassa));
            this.cassa.qtaIn = cassaw.qtaInc;
            this.cassa.valoreIn = cassaw.valoreInc;
            this.cassa.qtaAc = cassaw.qtaInc;
            this.cassa.valoreAc = cassaw.valoreInc;
            this.importoCassa = this.importoCassa + this.cassa.valoreIn;
            this.aggiornaTagliaCassa(this.cassa, this.idGiornata);
              }
           },
    error => {
         alert('sono in aggiornaCassaIniziale');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('aggiornaCassaIniziale - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
async salvaimportoCassa(id: number, tipo: string) {
  let rc = await this.cassawService.getTotaleCassa(id).subscribe(
    resp => {
          if(resp['rc'] === 'ok') {
            if(tipo === 'I') {
              this.giornata.cassaInizio = resp['incassato'];
              this.giornata.cassaAttuale = resp['incassato'];
              this.giornata.statoCassa = 1;
            }
            if(tipo === 'F') {
              this.giornata.cassaFinale = resp['incassato'];
              this.giornata.statoCassa = 3;
            }
            console.log('salvaimportoCassa ----------  incassato ----- CassaInizio ----------' + resp['incassato'] + ' this.giornata.cassaInizio' + this.giornata.cassaInizio);
            this.aggiornacassaGiorno(this.giornata);
            }
           },
    error => {
         alert('sono in salvaimportoCassa');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('salvaimportoCassa - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
async aggiornaCassaFinale(cassaw: Cassaw, id: number) {
  console.log('aggiornaCassaFinale - appena entrato');
  let rc = await this.cassaService.getbyidTagliaeGiorno(id, cassaw.idTaglia).subscribe(
    resp => {
          if(resp['rc'] === 'ok') {
            this.cassa = resp['data'];
            console.log('aggiornaCassaFinale ----------------------- cassa ' + JSON.stringify(this.cassa));
            this.cassa.qtaFi = cassaw.qtaInc;
            this.cassa.valoreFi = cassaw.valoreInc;
            this.cassa.qtaSb  = this.cassa.qtaFi - this.cassa.qtaIn;
            this.cassa.valoreSb = this.cassa.valoreFi - this.cassa.valoreIn;
            this.importoCassa = this.importoCassa + this.cassa.valoreFi;
            this.aggiornaTagliaCassa(this.cassa, this.idGiornata);
              }
           },
    error => {
         alert('sono in aggiornaCassaFinale');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('aggiornaCassaFinale - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
async aggiornacassaGiorno(giornata: Giornata) {
  ...
  let rc = await this.giornataService.updateGiornata(giornata).subscribe(
    ...
    error => {
         alert('sono in aggiornacassaGiorno');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('aggiornacassaGiorno - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
async creanewCassawGiornata(id: number) {
  ...
  let rc = await this.tagliaService.getAll().subscribe(
    ...
    error => {
         alert('creanewCassawGiornata error');
         this.isVisible = true;
         this.alertSuccess = false;
...
// ------------------------------------------------------------------------------  per caricare cassaw  da cassa   -- inizio
// nuova operatività per utilizzare la cassa e non i dati in giornata 23/02/2022
async loadCassawfromCassa(tipoCassa: string, id: number) {
  ...
  let rc = await this.cassaService.getbyidGiornata(id).subscribe(
    resp => {
          if(resp['rc'] === 'ok') {
            ...
            console.log('SalvoCassaIniziale ----------------------- cassa ' + JSON.stringify(this.casse));
            for(let cassa of this.casse) {
              this.cassaw = new Cassaw();
              if(tipoCassa === 'I') {
                  this.cassaw.idGiornata = id;
                  this.cassaw.idTaglia = cassa.idTaglia;
                  this.cassaw.qtaInc = cassa.qtaAc;
                  this.cassaw.valoreInc = cassa.valoreAc;
               }
              if(tipoCassa === 'F') {
                this.cassaw.idGiornata = id;
                this.cassaw.idTaglia = cassa.idTaglia;
                this.cassaw.qtaInc = cassa.qtaFi;
                this.cassaw.valoreInc = cassa.valoreFi;
               }
              this.aggiornaTagliaCassaw(this.cassaw, this.idGiornata);
              }
            }
...
async aggiornaTagliaCassaw(cassaw: Cassaw, id: number) {

  let rc = await this.cassawService.updateCassa(cassaw, id).subscribe(
    resp => {
          console.log(`aggiornaTagliaCassaw: ----------------------->  ${JSON.stringify(resp['data'])} `);
          if(resp['rc'] === 'ok') {
              console.log('Taglia Cassa ' + cassaw.idTaglia + '  Aggiornata correttamente');
            }
       },
    error => {
         alert('sono in aggiornaTagliaCassaw');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('aggiornaTagliaCassaw - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
// ------------------------------------------------------------------------------  per caricare cassaw  da cassa   -- fine

// --------------------   lettura cassaw per editazione in griglia  -- inizio

//  vecchio metedo non più utilizzato
// dal 23/02/2022  carico i dati da cassa
...
// --------------------   lettura cassaw per editazione in griglia  -- fine


async aggiornaTagliaCassa(cassa: Cassa, id: number) {

  console.log(`aggiornaTagliaCassa: -------- prima di update --------------->  ${JSON.stringify(cassa)} `);

  let rc = await this.cassaService.updateCassa(cassa, id).subscribe(
    resp => {

          if(resp['rc'] === 'ok') {
              console.log('Taglia Cassa ' + cassa.idTaglia + '  Aggiornata correttamente');
            }
       },
    error => {
         alert('sono in aggiornaTagliaCassa');
         this.isVisible = true;
         this.alertSuccess = false;
         console.log('aggiornaTagliaCassa - errore: ' + error);
         this.type = 'error';
         this.Message = error.message;
         this.alertSuccess = false;
         this.showNotification(this.type, this.Message);
...
/*
  per fare aggornamento  -- da sistemare
          ...
          if(resp['rc'] === 'ok') {
            ...
            for(let cassa of this.casse) {
              ...
              if(tipoCassa === 'I') {
                  this.cassaw.qtaInc = cassa.qtaAc;
                  this.cassaw.valoreInc = cassa.valoreAc;
                  this.cassa.qtaAc = cassa.qtaIn;
                  this.cassa.valoreAc = cassa.valoreIn;
                  if(cassa.idTaglia === 6) {
                    this.cassa.qtaIn = 1;
                  }
               }
              if(tipoCassa === 'F') {
                this.cassa.qtaFi = cassa.qtaIn;
                this.cassa.valoreFi = cassa.valoreIn;
                this.cassa.qtaSb = cassa.qtaFi - cassa.qtaAc;
                this.cassa.valoreSb = cassa.valoreFi - cassa.valoreAc;
               }
              this.aggiornaTagliaCassa(this.cassa, this.idGiornata);
              }
            }
...
/*   metodi non piu usati. utilizzavo la cassa su giornata
