...
ngOnInit(): void {
  ...
  this.loadUsers(this.ricercaIniziale);
...
async loadabilitazioni(abil: number) {
   ...
   const rc = await  this.abilfunctioneService.getAbilbylevel(abil).subscribe(
       response => {
          console.log('frontend - loadabilitazioni ' + JSON.stringify(response['data']));
...
async loadUsers(livello: number) {

  console.log('selezionato il livello: ' + livello);
  this.trovatoRec = false;
  this.isVisible = true;
  const rc = await  this.userService.getUserbyLevel(livello).subscribe(
      response => {
         console.log('frontend - loadUsers ' +  JSON.stringify(response['data']));
         this.nRecUser = response['number'];
     ...
     error => {
        alert('Abilifunctioni  -- loadUsers: ' + error.message);
        console.log(error);
        this.alertSuccess = false;
        this.Message = error.message;
